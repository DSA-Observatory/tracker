...
<div class="max-w-full min-w-0 overflow-hidden">
	<button
		bind:this={button}
		class={`flex h-9 w-full min-w-0 items-center justify-between gap-3 border border-slate-200 bg-white px-3 text-left text-sm font-medium text-slate-700 shadow-xs transition hover:bg-slate-50 focus-visible:ring-2 focus-visible:ring-slate-950/10 focus-visible:outline-none ${variant === 'collapsible' && open ? 'rounded-t-md' : 'rounded-md'}`}
		type="button"
		aria-expanded={open}
		onclick={toggleMenu}
	>
	...
	{#if open && variant === 'collapsible'}
		<div
			class="max-w-full min-w-0 overflow-hidden rounded-b-md border border-t-0 border-slate-200 bg-white p-2 shadow-xs"
			role="region"
			aria-label={`${label} filters`}
		>
			<input
				class="mb-2 h-9 w-full min-w-0 rounded-md border border-slate-200 bg-white px-3 text-sm text-slate-950 transition outline-none placeholder:text-slate-400 focus:border-slate-300 focus:ring-2 focus:ring-slate-950/10"
				type="search"
				bind:value={query}
				{placeholder}
				aria-label={placeholder}
			/>
			<div class="max-h-56 min-w-0 overflow-x-hidden overflow-y-auto pr-1">
				{#each visibleOptions as option (option.value)}
					<label
						class="flex min-w-0 cursor-pointer items-center justify-between gap-3 rounded-md px-2 py-2 text-sm transition hover:bg-slate-50"
					>
						<span class="flex min-w-0 items-center gap-2">
							<input
								class="size-4 rounded border-slate-300 text-slate-950 accent-slate-950"
